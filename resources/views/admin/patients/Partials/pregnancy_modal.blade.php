@foreach($patient->pregnancyRecords as $pr)
<div class="modal fade" id="viewPregnancyModal_{{ $pr->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title fw-bold"><i class="fas fa-baby me-2"></i>Pregnancy Record - {{ $patient->first_name }} ({{ $pr->created_at->format('Y') }})</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            
            <div class="modal-body p-0 bg-white">
                <form action="{{ route('admin.patients.update_pregnancy', $patient->id) }}" method="POST" id="pregnancyForm_{{ $pr->id }}">
                    @csrf
                    @method('PUT')
                    
                    <input type="hidden" name="redirect_to_calendar" id="redirect_to_calendar_{{ $pr->id }}" value="0">
                    
                    {{-- COMPLETION STATUS BANNER --}}
                    <div class="bg-white border-bottom p-3 d-flex justify-content-between align-items-center">
                        <div>
                            @if($pr->is_completed)
                                <span class="badge bg-success py-2 px-3 fs-6 shadow-sm"><i class="fas fa-check-circle me-1"></i> Pregnancy Term Completed</span>
                                <p class="text-muted small mb-0 mt-1">This patient has successfully concluded this pregnancy tracking term.</p>
                            @else
                                <span class="badge bg-warning text-dark py-2 px-3 fs-6 shadow-sm"><i class="fas fa-spinner fa-spin me-1"></i> Active Pregnancy</span>
                                <p class="text-muted small mb-0 mt-1">Track prenatal visits and vaccinations below.</p>
                            @endif
                        </div>

                        {{-- Mark as Complete Button --}}
                        @if(!$pr->is_completed)
                            <button type="submit" formaction="{{ route('admin.patients.complete_pregnancy', $patient->id) }}" class="btn btn-success fw-bold shadow-sm" onclick="return confirm('Are you sure you want to mark this pregnancy as completed? You will no longer need to track visits for this term.');">
                                <i class="fas fa-check-double me-1"></i> Mark Term as Complete
                            </button>
                        @endif
                    </div>
                    ...
                    <div class="bg-light border-bottom px-4 pt-3">
                        <ul class="nav nav-tabs nav-tabs-custom border-0" id="pregnancyTabs_{{ $pr->id }}" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active fw-bold text-secondary" id="general-tab_{{ $pr->id }}" data-bs-toggle="tab" data-bs-target="#general_{{ $pr->id }}" type="button" role="tab"><i class="fas fa-info-circle me-1"></i> General</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold text-secondary" id="prenatal-tab_{{ $pr->id }}" data-bs-toggle="tab" data-bs-target="#prenatal_{{ $pr->id }}" type="button" role="tab"><i class="fas fa-calendar-check me-1"></i> Prenatal (BANC)</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold text-secondary" id="supplements-tab_{{ $pr->id }}" data-bs-toggle="tab" data-bs-target="#supplements_{{ $pr->id }}" type="button" role="tab"><i class="fas fa-pills me-1"></i> Meds & Vaccines</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold text-secondary" id="labs-tab_{{ $pr->id }}" data-bs-toggle="tab" data-bs-target="#labs_{{ $pr->id }}" type="button" role="tab"><i class="fas fa-microscope me-1"></i> Laboratories</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold text-secondary" id="delivery-tab_{{ $pr->id }}" data-bs-toggle="tab" data-bs-target="#delivery_{{ $pr->id }}" type="button" role="tab"><i class="fas fa-procedures me-1"></i> Delivery & Postpartum</button>
                            </li>
                        </ul>
                    </div>

                    {{-- FIELDSET TO LOCK THE ENTIRE FORM IF COMPLETED --}}
                    <fieldset {{ $pr->is_completed ? 'disabled' : '' }} style="border: none; margin: 0; padding: 0; min-width: 0;">
                        <div class="tab-content p-4" id="pregnancyTabsContent_{{ $pr->id }}">
                            
                            <div class="tab-pane fade show active" id="general_{{ $pr->id }}" role="tabpanel">
                            ...
                            <div class="tab-pane fade" id="prenatal_{{ $pr->id }}" role="tabpanel">
                                        ...
                                        <div class="p-3 bg-success bg-opacity-10 border border-success border-opacity-25 rounded d-flex align-items-center justify-content-between">
                                            ...
                                            <div>
                                                <div class="form-check form-check-inline mb-0">
                                                    <input class="form-check-input" type="radio" name="completed_8anc" id="anc_yes_{{ $pr->id }}" value="1" {{ old('completed_8anc', $pr->completed_8anc ?? '0') == '1' ? 'checked' : '' }}>
                                                    <label class="form-check-label text-success fw-bold" for="anc_yes_{{ $pr->id }}">Yes</label>
                                                </div>
                                                <div class="form-check form-check-inline mb-0">
                                                    <input class="form-check-input" type="radio" name="completed_8anc" id="anc_no_{{ $pr->id }}" value="0" {{ old('completed_8anc', $pr->completed_8anc ?? '0') == '0' ? 'checked' : '' }}>
                                                    <label class="form-check-label text-danger" for="anc_no_{{ $pr->id }}">No</label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="tab-pane fade" id="supplements_{{ $pr->id }}" role="tabpanel">
                                ...
                                <div class="p-3 bg-light rounded border border-secondary border-opacity-10 mb-4">
                                    ...
                                    <div class="d-flex align-items-center bg-white p-2 rounded border shadow-sm">
                                        ...
                                        <input type="radio" name="ifa_completed" id="ifa_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('ifa_completed', $pr->ifa_completed ?? '0') == '1' ? 'checked' : '' }}><label class="small text-success fw-bold me-3" for="ifa_yes_{{ $pr->id }}">Yes</label>
                                        <input type="radio" name="ifa_completed" id="ifa_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('ifa_completed', $pr->ifa_completed ?? '0') == '0' ? 'checked' : '' }}><label class="small text-danger fw-bold me-3" for="ifa_no_{{ $pr->id }}">No</label>
                                ...
                                <div class="p-3 bg-light rounded border border-secondary border-opacity-10 mb-4">
                                    ...
                                    <div class="d-flex align-items-center bg-white p-2 rounded border shadow-sm">
                                        ...
                                        <input type="radio" name="mms_completed" id="mms_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('mms_completed', $pr->mms_completed ?? '0') == '1' ? 'checked' : '' }}><label class="small text-success fw-bold me-3" for="mms_yes_{{ $pr->id }}">Yes</label>
                                        <input type="radio" name="mms_completed" id="mms_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('mms_completed', $pr->mms_completed ?? '0') == '0' ? 'checked' : '' }}><label class="small text-danger fw-bold me-3" for="mms_no_{{ $pr->id }}">No</label>
                                ...
                                <div class="row g-4">
                                    <div class="col-md-8">
                                        <div class="p-3 bg-light rounded border border-danger border-opacity-25 h-100">
                                            ...
                                            <div class="d-flex align-items-center bg-white p-2 rounded border shadow-sm">
                                                ...
                                                <input type="radio" name="cc_completed" id="cc_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('cc_completed', $pr->cc_completed ?? '0') == '1' ? 'checked' : '' }}><label class="small text-success fw-bold me-2" for="cc_yes_{{ $pr->id }}">Yes</label>
                                                <input type="radio" name="cc_completed" id="cc_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('cc_completed', $pr->cc_completed ?? '0') == '0' ? 'checked' : '' }}><label class="small text-danger fw-bold me-2" for="cc_no_{{ $pr->id }}">No</label>
                                    ...
                                    <div class="col-md-4">
                                        <div class="p-3 bg-light rounded border border-secondary border-opacity-10 h-100 d-flex flex-column justify-content-center align-items-center text-center">
                                            ...
                                            <div>
                                                <input type="radio" name="deworming" id="deworm_yes_{{ $pr->id }}" value="1" class="btn-check" {{ old('deworming', $pr->deworming ?? '0') == '1' ? 'checked' : '' }}>
                                                <label class="btn btn-outline-success btn-sm px-4 fw-bold me-2" for="deworm_yes_{{ $pr->id }}">Yes (1)</label>
                                                
                                                <input type="radio" name="deworming" id="deworm_no_{{ $pr->id }}" value="0" class="btn-check" {{ old('deworming', $pr->deworming ?? '0') == '0' ? 'checked' : '' }}>
                                                <label class="btn btn-outline-danger btn-sm px-4 fw-bold" for="deworm_no_{{ $pr->id }}">No (0)</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="tab-pane fade" id="labs_{{ $pr->id }}" role="tabpanel">
                            ...
                            <div class="tab-pane fade" id="delivery_{{ $pr->id }}" role="tabpanel">
                                ...
                                <div class="row g-4">
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-6 d-flex align-items-center mt-4">
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" name="bemonc_cemonc" id="bemonc_cemonc_{{ $pr->id }}" value="1" {{ old('bemonc_cemonc', $pr->bemonc_cemonc ?? '0') == '1' ? 'checked' : '' }}>
                                                        <label class="form-check-label small fw-bold mt-1 ms-2" for="bemonc_cemonc_{{ $pr->id }}">BEmONC/CEmONC Capable</label>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="col-lg-5">
                                        ...
                                        <div class="d-flex align-items-center bg-light p-2 rounded border shadow-sm mb-4">
                                            ...
                                            <input type="radio" name="completed_4pnc" id="pnc_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('completed_4pnc', $pr->completed_4pnc ?? '0') == '1' ? 'checked' : '' }}><label class="small text-success fw-bold me-3" for="pnc_yes_{{ $pr->id }}">Yes</label>
                                            <input type="radio" name="completed_4pnc" id="pnc_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('completed_4pnc', $pr->completed_4pnc ?? '0') == '0' ? 'checked' : '' }}><label class="small text-danger fw-bold" for="pnc_no_{{ $pr->id }}">No</label>
                                        </div>
                                        ...
                                        <div class="p-3 bg-light rounded border border-secondary border-opacity-10 mb-3">
                                            ...
                                            <div class="d-flex align-items-center bg-white p-2 rounded border shadow-sm">
                                                ...
                                                <input type="radio" name="pp_ifa_completed" id="pp_ifa_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('pp_ifa_completed', $pr->pp_ifa_completed ?? '0') == '1' ? 'checked' : '' }}><span class="small text-success fw-bold me-2">Yes</span>
                                                <input type="radio" name="pp_ifa_completed" id="pp_ifa_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('pp_ifa_completed', $pr->pp_ifa_completed ?? '0') == '0' ? 'checked' : '' }}><span class="small text-danger fw-bold me-2">No</span>
                                        ...
                                        <div class="p-3 bg-light rounded border border-secondary border-opacity-10 d-flex flex-column justify-content-between">
                                            <div>
                                                ...
                                                <div class="d-flex align-items-center bg-white p-2 rounded border shadow-sm mb-3">
                                                    ...
                                                    <input type="radio" name="pp_vita_completed" id="vita_yes_{{ $pr->id }}" value="1" class="form-check-input me-1" {{ old('pp_vita_completed', $pr->pp_vita_completed ?? '0') == '1' ? 'checked' : '' }}><label class="small text-success fw-bold me-2" for="vita_yes_{{ $pr->id }}">Yes</label>
                                                    <input type="radio" name="pp_vita_completed" id="vita_no_{{ $pr->id }}" value="0" class="form-check-input me-1" {{ old('pp_vita_completed', $pr->pp_vita_completed ?? '0') == '0' ? 'checked' : '' }}><label class="small text-danger fw-bold me-2" for="vita_no_{{ $pr->id }}">No</label>
                    ...
                    <div class="modal-footer bg-light border-top p-3 d-flex justify-content-between">
                        @if($pr->is_completed)
                            <span class="text-success small fw-bold"><i class="fas fa-lock me-1"></i>This record is completed and locked from further edits.</span>
                            <div>
                        ...
                        @else
                            ...
                            <div>
                                <button type="button" class="btn btn-secondary rounded-pill px-4 me-2 shadow-sm" data-bs-dismiss="modal">Cancel</button>
                                <button type="button" class="btn btn-danger rounded-pill px-4 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#bookNextVisitModal_{{ $pr->id }}">
...
{{-- BOOK NEXT VISIT CONFIRMATION (One for each modal) --}}
@if(!$pr->is_completed)
<div class="modal fade" id="bookNextVisitModal_{{ $pr->id }}" tabindex="-1" aria-hidden="true" style="z-index: 1060;">
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4 bg-white text-center">
                ...
                <div class="d-flex flex-column gap-2 mt-2">
                    <button type="button" class="btn btn-primary fw-bold rounded-pill shadow-sm" onclick="submitAndRedirect('{{ $pr->id }}')">
                        <i class="fas fa-calendar-alt me-2"></i>Yes, Save & Go to Calendar
                    </button>
                    <button type="button" class="btn btn-light border fw-bold rounded-pill text-secondary" onclick="submitOnly('{{ $pr->id }}')">
                        No, Just Save Record
                    </button>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize all pregnancy forms on the page
        ...
        // Initialize dynamically if opened via modal
    ...
    // --- DYNAMIC SUBMISSION FOR LOOPED MODALS ---
    function submitAndRedirect(formId) {
        document.getElementById('redirect_to_calendar_' + formId).value = '1';
        document.getElementById('pregnancyForm_' + formId).submit();
    }

    function submitOnly(formId) {
        document.getElementById('redirect_to_calendar_' + formId).value = '0';
        document.getElementById('pregnancyForm_' + formId).submit();
    }
</script>